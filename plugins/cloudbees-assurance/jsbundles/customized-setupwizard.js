...
require('jenkins-js-modules').whoami('undefined:customized-setupwizard');
...
onSetupWizardInitialized(function(wizard) {
    ...
    var $bs = require('jenkins-js-modules').require('core-assets/bootstrap:bootstrap3').getBootstrap();
    var pluginManager = wizard.pluginManager;

    var Handlebars = wizard.jenkins.initHandlebars();

    // Override pluginSelectList.hbs template as the wizard does not expose it.
    var pluginSelectList = require('./templates/pluginSelectListOverriden.hbs');
    // Override pluginSelectionList.hbs template to include tier information.
    ...
    // Helper to get a tier for a given plugin
    Handlebars.registerHelper('getTier', function(tiers, name) {
        if (tiers === undefined) {
            tiers = loadPluginTierMap();
        }
        return tiers[name];
    });

    // Override the '.install-custom' action defined in the OSS wizard
    wizard.addActions({
        '.install-custom': function() {
            loadCustomPluginPanel();
        }
    });

    // This method overrides the behaviour defined in OSS.
    // Same behaviour as in OSS but customizing the data passed to the handlebars template to add the tier information.
    var loadCustomPluginPanel = function() {
        wizard.loadPluginCategories(function(){
            var data = wizard.pluginSelectionPanelData();
            var tiers = { 'tiers': loadPluginTierMap() };
            wizard.setPanel(pluginSelectionPanel, $.extend(data, tiers), function() {
                $bs('.plugin-selector .plugin-list').scrollspy({ target: '.plugin-selector .categories' });
            });
        });
    };

    // Generate tier information map given the platform plugins JSON
    var loadPluginTierMap = function() {
        var  plugins = {}
        pluginManager.plugins().forEach(function(pluginCateogry) {
            pluginCateogry.plugins.forEach(function(plugin) {
                if (plugin.tier) {
                    plugins[plugin.name] = plugin.tier
                }
            });
        });
        return plugins;
		...
		require('jenkins-js-modules').export(undefined, 'customized-setupwizard', {});
    });


},{"./templates/pluginSelectListOverriden.hbs":11,"./templates/pluginSelectionPanel.hbs":12,"jenkins-js-modules":6}],11:[function(require,module,exports){
...
module.exports = HandlebarsCompiler.template({"1":function(depth0,helpers,partials,data,blockParams,depths) {
  ...
  return "  <div class=\"plugin tier-"
    + alias2((helpers.getTier || (depth0 && depth0.getTier) || alias1).call(depth0,(depths[2] != null ? depths[2].tiers : depths[2]),((stack1 = (depth0 != null ? depth0.plugin : depth0)) != null ? stack1.name : stack1),{"name":"getTier","hash":{},"data":data}))
    + " "
