...
require('jenkins-js-modules').whoami('undefined:registration');
...
onSetupWizardInitialized(function(wizard) {
    ...
    var translations;
    wizard.translationOverride(function(allTranslations) {
        translations = allTranslations;
    });
    
    var $ = require('jenkins-js-modules').require('core-assets/bootstrap:bootstrap3').getBootstrap();
    ...
    var showStatePanel = function() {
        isRegistrationState = false;
        window.location.hash = null;
        wizard.showStatePanel();
    };
    
    $(document).on('click', '.install-home', function() {
        if (window.location.hash) {
            window.location.hash = '';
        }
    });
    
    var registrationPanel = require('./templates/registrationPanel.hbs');
    var modalPanel = require('./templates/modalPanel.hbs');
    
    var registrationHtml;
    var showRegistrationPanel = function() {
        if (!registrationHtml) {
            // TODO may need to get resource bundle, but need fix for JENKINS-35270
            //jenkins.loadTranslations('hudson.license.RegistrationHandler._modern', function(localizations) {
            //    $.extend(translations, localizations);
                jenkins.get('/registration/handler/registration', function(html) {
                    registrationHtml = registrationPanel({translations: translations});
                    var $registrationHtml = $(registrationHtml);
                    var $content = $registrationHtml.find('div[data-load-content]');
                    $content.removeAttr('data-load-content').html(html);
                    registrationHtml = $('<div/>').append($registrationHtml).html();
                    registrationPanel = function() {
            ...
            $('.modal-content').removeClass('registration-state');
            var id = '' + window.location.hash;
            if(id && id.length > 1) {
                id = id.substring(1);
                showModalForm(id);
            } else {
                wizard.setPanel(registrationPanel, {}, function() {
                    $('.modal-content').addClass('registration-state');
                });
            }
        }
    };
    ...
    wizard.addActions({
        '.skip-registration': function() {
            jenkins.goTo('/setupWizard/installState/REGISTRATION/hideUpgradeWizard');
        },
        '.cb-registration-form button:not(.install-home)': function() {
            var success = function(response) {
                if (response.status >= 400) {
                    $('.has-error').removeClass('has-error');
                    $('.error-message').remove();
                    // Have a spot to hold global errors; don't create until needed
                    var $generalError = $('.general-error');
                    var $getOrCreateGeneralError = function() {
                        if ($generalError.length === 0) {
                            $generalError = $('<div class="alert alert-danger general-error error-message"></div>');
                            $generalError.insertBefore('form .form-group:first');
                        }
                        return $generalError;
                    };
                    var rsp;
                    try {
                        ...
                        $getOrCreateGeneralError().text(response.responseText);
                        $getOrCreateGeneralError().prepend("Unable to parse response: " + e.message + "<br>");
                    }
                    for (var key in rsp) {
                        var $err = $('<div class="control-label error-message fade">' + rsp[key] + '</div>');
                        var $input = $('*[name="'+key+'"]');
    
                        if ($input.size() > 0) {
                            var $parent = $input.parent();
                            $parent.addClass('has-error');
                            $err.insertBefore($input);
                        } else {
                            // There's no input having the name specified in the error message.
                            // Lets add the error after the form inside the welcome panel.
                            $getOrCreateGeneralError().append($err);
                        }
                    }

                    var $showElement = $('.error-message:first');

                    if ($generalError.length === 0) {
                        var $formHasErrors = $('<div class="control-label error-message fade">' + 'Please correct the errors and try again' + '</div>')
                        $getOrCreateGeneralError().append($formHasErrors);
                    }

                    var $el = $showElement;
                    var top = 0;
                    while($el && $el.is(':not(.modal-body)') && $el.parents('.modal-body').length > 0) {
                        top += $el.position().top;
                        $el = $el.offsetParent();
                    }
                    top += $('.modal-body').scrollTop();
                    $('.modal-body').animate({
                        scrollTop: top
                    }, 250);

                    $('.fade:not(.in)').addClass('in');
                } else {
                    // success!
                    var curElement = document.activeElement;
                    if (document.getElementById('remove-components-button') == curElement) {
                        //CJP-6662  
                        jenkins.goTo('/');
                    } else {
                        showStatePanel();
    ...
    var showModalForm = function(id) {
        var $form = $($(registrationHtml).find('#'+id).html());
        $form.find('*').removeClass(function (index, css) {
            return (css.match (/^(row|col.*)/g) || []).join(' ');
        });
        $form.find('input[type=text],input[type=password],input[type=number],textarea').addClass('form-control');
        var url = $form.attr('action');
        $form.find('.close').remove();
        if ($form.find('input[name=json]').length === 0) {
            $form.append('<input type="hidden" name="json"/>');
        }
        ...
        var formPanel = function(ctx) {
            return modalPanel($.extend({title: title, content: $form.html(), buttons: $buttons.html()}, ctx));
        };
        wizard.setPanel(formPanel);
    };
    ...
    // Integrate the existing forms into the wizard more naturally
    $(document).on('click', '.licChoice .btn.pop', function(e) {
        var id = $(this).attr('data-form-id');
        window.location.hash = id;
    });
});

// TODO remove this section once the fix for JENKINS-33374 is in the Jenkins core baseline
onSetupWizardInitialized(function(wizard) {
    var $ = require('jenkins-js-modules').require('core-assets/bootstrap:bootstrap3').getBootstrap();
    var jenkins = wizard.jenkins; // wizard-provided jenkins api
    var pluginManagerErrorTimeoutMillis = 10*1000; // 10 sec
    ...
    wizard.translationOverride(function(allTranslations) {
        ...
        // Due to JENKINS-35270 we cannot load translations from plugins
        var text = {
            'en': {
                installWizard_error_restartNotSupported:'Restart is not supported, please manually restart this instance',
                installWizard_installComplete_bannerRestart:translations.installWizard_cjpVersionTitle+' is almost ready!',
                installWizard_installComplete_installComplete_restartRequiredMessage:'Setup is complete, but some plugins require '+translations.installWizard_cjpVersionTitle+' to be restarted.',
                installWizard_installComplete_installComplete_restartRequiredNotSupportedMessage:'Setup is complete, but some plugins require '+translations.installWizard_cjpVersionTitle+' to be restarted and this instance does not support an automated restart. Please manually restart your instance now to complete installation.',
            },
        };
        if (!text[language]) {
            language = 'en';
        }
        $.extend(translations, text[language]);
    });
    
    var restartStatus = function(handler) {
        jenkins.get('/registration/handler/restartStatus', function(response) {
            handler.call({ isError: false }, response.data ? response.data : response);
        }, {
    ...
    var showSetupCompletePanel = function(messages) {
        restartStatus(function(restartStatus) {
            wizard.setPanel(setupCompletePanel, $.extend(restartStatus, messages));
        });
    };

    var skipFirstUser = function() {
        $('button').prop({disabled:true});
        wizard.showStatePanel('CONFIGURE_INSTANCE');
    };
    ...
    var restartJenkins = function() {
        var pluginManager = wizard.pluginManager;
        pluginManager.completeInstall(function() {
            pluginManager.restartJenkins(function() {
                wizard.setPanel(loadingPanel);
    ...
    wizard.addActions({
		...
		require('jenkins-js-modules').export(undefined, 'registration', {});
    });


},{"./templates/loadingPanel.hbs":11,"./templates/modalPanel.hbs":12,"./templates/registrationPanel.hbs":13,"./templates/setupCompletePanel.hbs":14,"jenkins-js-modules":6}],11:[function(require,module,exports){
...
module.exports = HandlebarsCompiler.template({"compiler":[6,">= 2.0.0-beta.1"],"main":function(depth0,helpers,partials,data) {
  ...
  return "<div class=\"modal-header\">\n  <h4 class=\"modal-title\">\n    "
    ...
    + "\n  </div>\n</div>\n<div class=\"modal-footer cb-registration-form\">\n  <button type=\"button\" class=\"btn btn-link install-home\">\n    "
    + alias3(this.lambda(((stack1 = (depth0 != null ? depth0.translations : depth0)) != null ? stack1.installWizard_goBack : stack1), depth0))
    + "\n  </button>\n  "
    + ((stack1 = ((helper = (helper = helpers.buttons || (depth0 != null ? depth0.buttons : depth0)) != null ? helper : alias1),(typeof helper === alias2 ? helper.call(depth0,{"name":"buttons","hash":{},"data":data}) : helper))) != null ? stack1 : "")
    + "\n</div>\n";
},"useData":true});
...
var HandlebarsCompiler = require('jenkins-handlebars-rt/runtimes/handlebars3_rt');
module.exports = HandlebarsCompiler.template({"compiler":[6,">= 2.0.0-beta.1"],"main":function(depth0,helpers,partials,data) {
    return "<div class=\"modal-body\">\n    <div class=\"jumbotron welcome-panel registration-panel\" data-load-content=\"/registration/handler/registration\">\n    </div>\n</div>\n";
},"useData":true});

},{"jenkins-handlebars-rt/runtimes/handlebars3_rt":5}],14:[function(require,module,exports){
// hbsfy compiled Handlebars template
var HandlebarsCompiler = require('jenkins-handlebars-rt/runtimes/handlebars3_rt');
...
},{"jenkins-handlebars-rt/runtimes/handlebars3_rt":5}]},{},[10]);
